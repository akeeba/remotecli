...
namespace Akeeba\RemoteCLI\Encrypt;

use Akeeba\RemoteCLI\Encrypt\AesAdapter\AdapterInterface;
use Akeeba\RemoteCLI\Encrypt\AesAdapter\Mcrypt;
use Akeeba\RemoteCLI\Encrypt\AesAdapter\OpenSSL;
...
class Encrypt
{
	 ...
	 * @param   bool    $useLegacy  Use the legacy, wrong way to expand the key and determine an IV
	 *
	 * @return  string  The ciphertext
	 */
	public function AESEncryptCBC($plaintext, $password, $useLegacy = false)
	{
		...
		$iv = $useLegacy ? $this->createTheWrongIV($password) : $rand->generate($keySizeBytes);
		...
		// ...plus the IV at the end (20 bytes)... (unless legacy)
		if (!$useLegacy)
		{
			$ciphertext .= 'JPIV' . $iv;
		}
